...
async function getVideosRecursive(dir: string, baseDir: string): Promise<any[]> {
    ...
    for (const file of list) {
        ...
        if (stat && stat.isDirectory()) {
            ...
            results.push({ name: file, id: id, thumbnail: posterPath || `/thumbnails/${id}.png` });
        }
    }
    return results;
}
...
async function processNextInQueue() {
    if (thumbnailQueue.length === 0) { isProcessingQueue = false; return; }
    ...
    try {
        ...
        const ffmpegProcess = spawn('ffmpeg', ['-nostdin', '-ss', '10', '-i', absolutePath, '-frames:v', '1', '-s', '320x180', '-y', thumbnailPath]);
        const timeout = setTimeout(() => {
            ffmpegProcess.kill('SIGKILL');
            if (res && !res.headersSent) res.status(504).end();
            processNextInQueue();
        }, 30000);
        ffmpegProcess.on('close', (code: number) => {
            ...
            if (code === 0) {
                if (res && !res.headersSent) res.sendFile(thumbnailPath, () => {});
            } else { if (res && !res.headersSent) res.status(500).end(); }
            processNextInQueue();
        });
        ffmpegProcess.on('error', () => { clearTimeout(timeout); processNextInQueue(); });
    } catch (error) { processNextInQueue(); }
}
...
async function startBackgroundScan(videos: any[]) {
    ...
    for (const video of videos) {
        ...
        if (!fs.existsSync(thumbnailPath)) {
            if (!thumbnailQueue.some(t => t.id === video.id)) {
                thumbnailQueue.push({ id: video.id, filePath, thumbnailPath, filename: video.name });
            }
        }
    }
    if (!isProcessingQueue && thumbnailQueue.length > 0) processNextInQueue();
}
...
app.post('/api/set-media-dir', async (req: Request, res: Response) => {
    ...
    try { await fs.ensureDir(getThumbnailDir()); } catch (e) {}
...
app.get('/api/generate-thumbnail/:id', async (req: Request, res: Response) => {
    ...
    thumbnailQueue.unshift({ id: req.params.id, filePath, thumbnailPath, filename: path.basename(filePath), res });
